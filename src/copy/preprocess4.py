...
trainFeature.to_csv('{}/train_gf_1yr.csv'.format(dataFolder),index = False)
testFeature.to_csv('{}/test_gf_1yr.csv'.format(dataFolder),index = False)
...
def PdFrame2FileLine(DataFrame, categoricalVariables, nonCategoricalVariables, nonFeatures, separate_symbol = ';'):
  ...
  for feat in nonFeatures:
    ...
    total_line.append(dataColumn)
  for feat in nonCategoricalVariables:
    dataColumn = DataFrame[feat].to_string()
    dataColumn = [i.split()[1] for i in dataColumn.split('\n')]
    total_line.append(dataColumn)
  ...
  for feat in categoricalVariables:
    ...
    def create_one_hot_vector(indx):
      vec = [0] * leng
      vec[indx] = 1
      #return separate_symbol.join(vec)
      return vec
    dataColumn = list(map(create_one_hot_vector, dataColumn))
    total_line.append(dataColumn)
  return total_line, num_feat

t1 = time()
trainLine, num_feat1 = PdFrame2FileLine(trainFeature, categoricalVariables, nonCategoricalVariables, nonFeatures, separate_symbol = ';')
print('cost {} seconds'.format(time() - t1))
testLine, num_feat2 = PdFrame2FileLine(testFeature, categoricalVariables, nonCategoricalVariables, nonFeatures, separate_symbol = ';')
assert num_feat1 == num_feat2



train_data = np.zeros((Ntrain, num_feat1),dtype = float)
test_data = np.zeros((Ntest, num_feat1), dtype = float)
