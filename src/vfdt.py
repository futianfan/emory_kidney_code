...
class VfdtNode:
    ...
    def sort_example(self, x):  ## x is data sample 
        if self.is_leaf():
            return self
        else:
    ...
    def sort_example2(self, x, prefix = ''):
        if self.is_leaf():
            return '', prefix
        else:
            index = self.possible_split_features.index(self.split_feature)
            ...
            if isinstance(split_value, list):  # discrete value
                if value in split_value[0]:
                    return self.left_child.sort_example2(x, prefix + '0')

                else:
                    return self.right_child.sort_example2(x, prefix + '1')
            else:  # continuous value
                if value <= split_value:
                    return self.left_child.sort_example2(x, prefix + '0')
                else:
    ...
    def update_stats(self, x, y):  # x feature, y label; self is a node
...
class Vfdt:
    ...
    def update(self, x, y):  ## x is feature, y is label
...
def test_run():
    ...
    for i in range(n_training):
        root_node = tree.root
        _, indx = root_node.sort_example2(array[i,:-1], '')
        leaf_node_set[indx] += [i] 
    
    for k in leaf_node_set:
        print(str(k) + ' ' + str(len(leaf_node_set[k])))
